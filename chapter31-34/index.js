// ------------------- DATE METHODS -----------------------
// 1. Write a program that displays current date and time in your browser. 
let date = new Date();
document.writeln(`Current date and time is : ${date}`)
document.writeln(`<hr>`);
// 2. Write a program that alerts the current month in words. For example December.
let monthList = ['January','February','March','April','May','June','July','August','September','October','November','December']
let month = date.getMonth();
document.writeln(`Current Month is: ${monthList[month]}`)
document.writeln(`<hr>`);
// 3. Write a program that alerts the first 3 letters of the current day, for example if today is Sunday then alert will show Sun. 
let daysList = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat']
let day = date.getDay();
document.writeln(`Today is: ${daysList[day]}`)
document.writeln(`<hr>`);
// 4. Write a program that displays a message “It’s Fun day” if its Saturday or Sunday today. 
if(day===0 || day==6){
    document.writeln(`<h3>Woohooo! It's Fun Day</h3>`)
}else{
    document.writeln(`<h3>Again! It's a Boring Day</h3>`)
}
document.writeln(`<hr>`);
// 5. Write a program that shows the message “First fifteen days of the month” if the date is less than 16th of the month else shows “Last days of the month”. 
let currDate=date.getDate();
if(currDate<16){
 document.writeln(`<h3>First fifteen days od the month</h3>`)
}
else{
     document.writeln(`<h3>Last days of month</h3>`)
}
document.writeln(`<hr>`);
// 6. Write a program that determines the minutes since midnight, Jan. 1, 1970 and assigns it to a variable that hasn't been declared beforehand. Use any variable you like to represent the Date object. 
let ms = date.getTime();
let mins= Math.floor(ms/(1000*60));
document.writeln(`Elapsed milliseconds since January 1,1970 :${ms}<br>Elapsed minutes since January 1,1970 :${mins}`)
document.writeln(`<hr>`);
// 7. Write a program that tests whether it's before noon and alert “Its AM” else “its PM”. 
let time = date.getHours();
if(time >0 && time<=12){
    document.writeln(`<h3>It's AM</h3>`)
}
else{
    document.writeln(`<h3>It's PM</h3>`)
}
document.writeln(`<hr>`);
// 8. Write a program that creates a Date object for the last day of the last month of 2020 and assigns it to variable named laterDate.
let laterDate = new Date(2020, 12, 0);

document.writeln('Later date: '+ laterDate);
document.writeln(`<hr>`);
// 9. Create a date object of the starting date of this Ramadan and alert the number of days past since 1st Ramadan? Note: 1st Ramadan was on June 18, 2015
let startDate= new Date("June 18, 2015");
// Difference in milliseconds
var diff = date - startDate;

// Convert milliseconds → days
var daysPassed = Math.floor(diff / (1000 * 60 * 60 * 24));
document.writeln(`${daysPassed} days have passed since 1st ramadan, 2015`);
document.writeln(`<hr>`);
// 10. Write a program that displays in your browser the seconds that elapsed between the reference date and the beginning of 2015. 
// Reference date (Jan 1, 1970)
var refDate = new Date(0);

// Beginning of 2015
var start2015 = new Date("Jan 1, 2015");

// Difference in milliseconds
var diff = start2015 - refDate;

// Convert milliseconds → seconds
var seconds = Math.floor(diff / 1000);

// Display in browser
document.writeln("Reference date "+ refDate + ", "+seconds +" Seconds elapsed between Jan 1, 1970 and Jan 1, 2015: ");
document.writeln('<hr>');
// 11. Create a Date object for the current date and time. Extract the hours, reset the date object an hour ahead and finally display the date object in your browser. 
// Extract current hours
var hours = date.getHours();

// Add 1 hour
date.setHours(hours + 1);

// Display updated date
document.writeln("Updated Date & Time: " + date);
document.writeln('<hr>');

// 12. Write a program that creates a date object and show the date in an alert box that is reset to 100 years back? 

// date.setFullYear(date.getFullYear() - 100);
// alert('100 years back it was : '+date);
document.writeln('<hr>');
// 13. Write a program to ask the user about his age. Calculate and show his birth year in your browser. 
let age= +prompt('Enter your age');
 // Get current year
      let currentYear = date.getFullYear();

      // Calculate birth year
      let birthYear = currentYear - age;

      // Validate input
      if (isNaN(age) || age <= 0) {
        document.writeln("Please enter a valid age!");
      } else {
        document.writeln("Your birth year is: " + birthYear);
      }
    
document.writeln('<hr>');

// 14. Write a program to generate your K-Electric bill in your browser. All the amounts should be rounded off to 2 decimal places. Display the following fields: 
// a. Customer Name 
// b. Current Month 
// c. Number of units 
// d. Charges per unit 
// e. Net Amount Payable (within Due Date) 
// f. Late Payment Surcharge 
// g. Gross Amount Payable (after Due Date) 
// Where, Net Amount Payable (within Due Date) = Number of units * Charges per unit & Gross Amount Payable (after Due Date) = Net Amount + Late Payment Surcharge
 // Customer details
  var customerName = "Fariha Mansoor";
  var currentMonth = monthList[month];
  var units = 350;
  var chargesPerUnit = 24.50;

  // Calculations
  var netAmount = units * chargesPerUnit;
  var lateSurcharge = 500;
  var grossAmount = netAmount + lateSurcharge;

  // Display bill
  document.writeln("<h2>K-Electric Bill</h2>");
  document.writeln("Customer Name: " + customerName + "<br>");
  document.writeln("Month: " + currentMonth + "<br>");
  document.writeln("Number of Units: " + units + "<br>");
  document.writeln("Charges per Unit: " + chargesPerUnit.toFixed(2) + "<br><br>");

  document.writeln("Net Amount Payable (within Due Date): " + netAmount.toFixed(2) + "<br>");
  document.writeln("Late Payment Surcharge: " + lateSurcharge.toFixed(2) + "<br>");
  document.writeln("Gross Amount Payable (after Due Date): " + grossAmount.toFixed(2));
